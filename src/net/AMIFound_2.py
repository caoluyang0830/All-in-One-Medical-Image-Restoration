...
class MySequential(nn.Sequential):
    def forward(self, x1, x2=None):
        for layer in self:
            if isinstance(layer, (NestedModExpert, ModExpert)):
                x1 = layer(x1, x2)
            else:
                x1 = layer(x1)
        return x1
...
class InnerAdapterLayer(nn.Module):
    """内层混合专家适配器，作为第一层专家的子模块"""

    def __init__(self, dim: int, rank: int, num_experts: int = 2, top_k: int = 1,
                 expert_layer: nn.Module = FFTAttention, depth: int = 1):
        super().__init__()
        self.tau = 1
        self.loss = 0.0
        self.top_k = top_k
        ...
        # 内层专家参数
        patch_sizes = [2 ** (i + 1) for i in range(num_experts)]
        kernel_sizes = [3 + (2 * i) for i in range(num_experts)]
        depths = [depth for _ in range(num_experts)]
        ranks = [max(1, rank // 2) for _ in range(num_experts)]

        self.experts = nn.ModuleList([
            MySequential(*[ModExpert(dim, rank=r, func=expert_layer,
        ...
        expert_complexity = torch.tensor([sum(p.numel() for p in expert.parameters())
                                          for expert in self.experts])
        self.routing = InnerRoutingFunction(
    ...
    def forward(self, x, shared):
        ...
        if self.training:
            ...
            for exp in range(len(self.experts)):
                out, _ = self.experts[exp](expert_inputs[exp], expert_shared_intputs[exp])
...
class NestedModExpert(nn.Module):
    """嵌套式专家模块，包含内层混合专家"""

    def __init__(self, dim: int, rank: int, func: nn.Module, depth: int,
                 patch_size: int, kernel_size: int, inner_num_experts: int = 2):
        super(NestedModExpert, self).__init__()
        self.depth = depth
        self.proj_in = nn.Conv2d(dim, rank, kernel_size=1, padding=0, bias=False)
        self.proj_mid = nn.Conv2d(dim, rank, kernel_size=1, padding=0, bias=False)
        self.proj_out = nn.Conv2d(rank, dim, kernel_size=1, padding=0, bias=False)

        # 内层混合专家
        self.inner_moe = InnerAdapterLayer(
            rank,
            rank=rank // 2,  # 内层专家使用更小的秩
            num_experts=inner_num_experts,
            expert_layer=func,
            depth=depth // 2  # 内层专家使用更少的深度
        )
    ...
    def forward(self, x, shared):
        ...
        shortcut = x
        x = self.proj_in(x)
        shared_mid = self.proj_mid(shared)

        # 调用内层混合专家
        x, inner_loss = self.inner_moe(x, shared_mid)

        x = self.proj_out(x)
        return x + shortcut, inner_loss
...
class InnerRoutingFunction(nn.Module):
    """内层路由函数，简化版，不依赖频率嵌入"""

    def __init__(self, dim, num_experts, k, complexity, complexity_scale: str = "max"):
        super(InnerRoutingFunction, self).__init__()
        self.gate = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            Rearrange('b c 1 1 -> b c'),
            nn.Linear(dim, num_experts, bias=False)
...
class AdapterLayer(nn.Module):
    def __init__(self,
                 ...
                 freq_dim: int = 128, inner_num_experts: int = 2,  # 新增：内层专家数量
                 with_complexity: bool = False, complexity_scale: str = "min"):
        super().__init__()
        self.tau = 1
        self.loss = 0.0
        self.inner_loss = 0.0  # 新增：内层专家损失
        self.top_k = top_k
        self.noise_eps = 1e-2
        self.num_experts = num_experts
        ...
        # 使用嵌套式专家替换原来的基础专家
        self.experts = nn.ModuleList([
            MySequential(*[NestedModExpert(
                dim, rank=rank, func=expert_layer, depth=depth,
                patch_size=patch, kernel_size=kernel,
                inner_num_experts=inner_num_experts  # 传入内层专家数量
    ...
    def forward(self, x, freq_emb, shared):
        gates, top_k_indices, top_k_values, outer_loss = self.routing(x, freq_emb)
        self.loss = outer_loss
        self.inner_loss = 0.0  # 重置内层损失
        ...
        if self.training:
            ...
            for exp in range(len(self.experts)):
                # 外层专家返回输出和内层损失
                out, inner_loss = self.experts[exp](expert_inputs[exp], expert_shared_intputs[exp])
                expert_outputs.append(out)
                self.inner_loss += inner_loss  # 累加内层损失
            out = dispatcher.combine(expert_outputs, multiply_by_gates=True)
        else:
            selected_experts = [self.experts[i] for i in top_k_indices.squeeze(0)]
            expert_outputs = []
            for expert in selected_experts:
                out, _ = expert(x, shared)  # 推理时不计算内层损失
                expert_outputs.append(out)
            expert_outputs = torch.stack(expert_outputs, dim=1)
            gates = gates.gather(1, top_k_indices)
            weighted_outputs = gates.unsqueeze(2).unsqueeze(3).unsqueeze(4) * expert_outputs
        ...
        out = self.proj_out(out)
        # 总损失 = 外层损失 + 内层损失（按比例加权）
        total_loss = self.loss + 0.3 * (self.inner_loss / max(1, len(self.experts)))
        return out, total_loss


##########################################################################
## 编码器和解码器块 (保持不变，除了解码器块返回损失)
...
class DecoderBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type, expert_layer, complexity_scale=None,
                 ...
                 freq_dim: int = 128, inner_num_experts: int = 2, with_complexity: bool = False):
        super().__init__()
        ...
        self.adapter = AdapterLayer(
            dim, rank,
            top_k=top_k, num_experts=num_experts, expert_layer=expert_layer,
            freq_dim=freq_dim, inner_num_experts=inner_num_experts,  # 新增内层专家数量参数
    ...
    def forward(self, x, freq_emb=None):
        ...
        x_a = self.proj[1](x)
        x_s = self.shared(x_s)
        x_a, loss = self.adapter(x_a, freq_emb, x_s)  # 获取总损失
        ...
        for i in range(num_blocks):
            self.layers.append(
                DecoderBlock(
                    ...
                    stage_depth=stage_depth, freq_dim=freq_dim, inner_num_experts=inner_num_experts,  # 传入内层专家数量
                    complexity_scale=complexity_scale, depth_type=depth_type, rank_type=rank_type,
...
##########################################################################
## 嵌套式混合专家模型
class NestedAMIFound(nn.Module):
    def __init__(self,
                 inp_channels=3,
                 out_channels=3,
                 dim=32,
        ...
        super(NestedAMIFound, self).__init__()
        ...
        for i in range(levels - 1):
            self.dec.append(nn.ModuleList([
                ...
                DecoderResidualGroup(
                    ...
                    num_heads=heads[i + 1],
                    ffn_expansion=ffn_expansion_factor,
                    LayerNorm_type=LayerNorm_type, bias=bias, expert_layer=expert_layer,
                    freq_dim=dims[0], with_complexity=with_complexity, inner_num_experts=inner_num_experts,  # 传入内层专家数量
                    rank=ranks[i], num_experts=num_experts, stage_depth=stage_depth[i],
                    depth_type=depth_type, rank_type=rank_type, top_k=topk, complexity_scale=complexity_scale),
            ])
            )
    ...
    def forward(self, x, labels=None):
        ...
        for i, (upsample, fusion, block) in enumerate(self.dec):
            ...
            self.total_loss += block.loss  # 累加所有块的总损失（包含内外层）
...
if __name__ == "__main__":
    ...
    model = NestedAMIFound(
        ...
        depth_type="constant",
        rank_type="spread",
        num_experts=4,  # 外层专家数量
        inner_num_experts=2,  # 内层专家数量
        topk=1,
        expert_layer=FFTAttention
    ).cuda()
    ...
    output = model(x)
    print("总损失（包含内外层路由损失）:", model.total_loss)

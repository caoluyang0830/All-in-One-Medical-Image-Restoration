...
class SpatialAttentionExpert(nn.Module):
    """空间注意力子专家"""
    def __init__(self, dim: int, kernel_size: int = 3, patch_size: int = None):
        super().__init__()
        self.num_heads = 1
        self.temperature = nn.Parameter(torch.ones(self.num_heads, 1, 1))
        self.qkv = nn.Conv2d(dim, dim * 3, kernel_size=1, bias=False)
        self.qkv_dwconv = nn.Conv2d(dim * 3, dim * 3, kernel_size=kernel_size, stride=1, padding=kernel_size // 2,
                                    groups=dim * 3)
...
class LocalFeatureExpert(nn.Module):
    """局部特征子专家（ConvNeXt风格）"""
    def __init__(self, dim: int, kernel_size: int = 7, patch_size: int = None):
        super().__init__()
...
class FFT_CNN_FusionExpert(nn.Module):
    """频域-空间融合子专家"""
    def __init__(self, dim: int, kernel_size: int = 3, patch_size: int = 8):
        super().__init__()
...
class ModExpert(nn.Module):
    def __init__(self, dim: int, rank: int, func: nn.Module, depth: int, patch_size: int, kernel_size: int):
        ...
        self.depth = depth
        # 保持原投影层和接口参数不变（兼容AdapterLayer初始化）
        ...
        # -------------------------- 最小改动：内部集成4种子专家 --------------------------
        # 复用原func参数的签名（实际忽略，固定为4种类型），不改动外部传参逻辑
        self.sub_experts = nn.ModuleList([
            FFTAttention(rank, kernel_size=kernel_size, patch_size=patch_size),
            SpatialAttentionExpert(rank, kernel_size=kernel_size),
            LocalFeatureExpert(rank, kernel_size=kernel_size),
            FFT_CNN_FusionExpert(rank, kernel_size=kernel_size, patch_size=patch_size)
        ])
        # 子专家融合层（仅新增这1个层）
        self.sub_expert_fusion = nn.Conv2d(rank * 4, rank, kernel_size=1, bias=False)

    def process(self, x, shared):
        shortcut = x
        # 保持原投影和门控逻辑不变
        x = self.proj[0](x)
        shared_gate = F.silu(self.proj[1](shared))

        # -------------------------- 新增：4种子专家并行处理 --------------------------
        sub_outputs = [sub_expert(x) * shared_gate for sub_expert in self.sub_experts]
        x = torch.cat(sub_outputs, dim=1)  # 拼接：(b, rank*4, h, w)
        x = self.sub_expert_fusion(x)  # 融合为：(b, rank, h, w)
...
##########################################################################
##
class AMIFound_Multiexpers(nn.Module):
    def __init__(self,
                 inp_channels=3,
                 out_channels=3,
                 dim=32,
        ...
        super(AMIFound_Multiexpers, self).__init__()
...
if __name__ == "__main__":
    # test
    model = AMIFound(rank=2, num_blocks=[4, 6, 6, 8], num_dec_blocks=[2, 4, 4], levels=4, dim=48, num_refinement_blocks=4,
